...
folderList <- c(topoFileFolder, simPackage, results, randcompiled, randRaw, WTcoherence, 
...
setwd(topoFolder)
topoFiles <- list.files(topoFiles)
...
netNameKey <- sapply(topoFiles, function(x){
    ...
    nodes <- unique(df$Source, df$Target) %>% length
    edges <- nrow(df)
...
BmodelSetup <- function()
{
    ...
    script[1] <- paste0("include('", simPackage, "')")
